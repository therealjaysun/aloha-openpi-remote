...
- **Implementation tasks:** Retain existing metrics and add trajectory sample/joint counts, exact applied-step coverage, plot status, and safe local plot IDs. After each complete or partial episode, atomically render one PNG from valid step rows with elapsed time on X and fixed-range percent on Y; overlay 14 solid actual series grouped blue/orange by arm, thinner dashed commands, an external legend, and an honest symmetric-zero/home reference. Grippers use the simulator's documented normalized coordinate; note that its XML stops lie slightly inside the nominal close/open calibration.
