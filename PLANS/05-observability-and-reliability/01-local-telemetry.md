...
- **Implementation tasks:** Keep the existing line-buffered `step` event and exact 14-joint vectors. Reuse it for bounded stderr 10%/coverage status plus safe stage IDs; never print prompt text, paths, machine IDs, actions, or raw telemetry. At each staged boundary drain the old in-flight request, discard every old queued action, then fetch the new stage before applying its first step. Preserve partial rows and the no-fsync/no-network step path.
