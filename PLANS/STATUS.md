...
| S0827 Push-PI | Arm-explicit prompts implemented; exact-candidate revalidation pending | `codex/push-pi-scenarios` | 8 | [PR 8](https://github.com/therealjaysun/pi-robotics/pull/8) | `codex/06-hardening-docs` | `codex/push-pi-scenarios` | 141 focused tests and 385 non-rendering tests pass with one platform skip; prior 24-episode evidence is historical for the previous prompts | Run the clean Mac calibration/full gate, then both exact PC matrices | Prompt amendment at branch HEAD; prior hardware implementation `7c2ec592` |

## Active extension cursor

- S0827 Push-PI has arm-explicit prompts implemented on `codex/push-pi-scenarios`, stacked temporarily on `codex/06-hardening-docs` by explicit user direction.
- Mac state: focused and non-rendering gates pass; clean-checkout calibration and the full GUI-capable Mac gate remain pending for the amended prompt descriptor.
...
- Machine gate: **ON AFTER MAC GATE**. Rerun both exact-SHA 12-episode matrices because the language input changed; prior matrix evidence remains historical.
