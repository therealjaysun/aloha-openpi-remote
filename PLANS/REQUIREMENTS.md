...
The arm-explicit prompt amendment reopens only S0827 hardware validation, not phases 00–06. The amended exact Mac candidate passes; E-MAC-S0827 and E-PC-S0827 remain historical evidence for the prior prompts until final evidence is recorded.

| ID | Requirement | Owner | Evidence | Status |
| --- | --- | --- | --- | --- |
| SP01 | Identify the work as custom PushT-inspired Push-PI, never as a standard PushT benchmark or comparable score | S0827 | docs/source review | Pass: amended Mac gate |
| SP02 | Preserve Transfer Cube as default; accept only the fixed `ALOHA_SCENARIO` enum and `ALOHA_DISPLAY=0|1` | S0827 | config tests | Pass: amended Mac gate |
| SP03 | One project-owned Gymnasium implementation registers four fixed 300-step/50 Hz IDs without editing site-packages | S0827 | pure + Mac simulator tests | Pass: amended Mac gate |
| SP04 | Preserve finite ALOHA `(14,)` observation/applied-action and `(50,14)` model-response contracts with fixed arm-explicit top-camera prompt input | S0827 | contract tests + profile smoke | Mac pass; amended PC smoke/matrices pending |
| SP05 | A committed descriptor freezes source-derived table bounds, geometry/physics, target/spawn region, pusher value, park tolerance, display cadence, outcome thresholds, and scene hash before GPU work | S0827 | Mac calibration evidence | Pass: amended Mac gate |
...
| SP12 | Scenario telemetry remains below 1 ms write p95 and raw JSONL/videos/plots/manifests remain ignored | S0827 | benchmark + Git checks | Mac pass; amended hardware measurement pending |
...
| --- | --- | --- | --- |
| E-MAC-S0827 | SP01–SP09, SP12–SP14 | Mac + GitHub-hosted Linux; `2026-08-28T23:52Z`; exact implementation `7c2ec5927ad200e5aaf30bed0db4ef61cb9e2ba4`; upstream `215abfb217dbac7d5f1273282331b9b1866c0479`; descriptor `9942fa7e851e352c2e7717e84a9fe1cb66b07f61c96fa62d9df136cefc6b154b`. `make scenario-calibrate`, the overview's exact stock/custom hold/display smokes, `make ci`, `make secret-scan`, `make public-audit`, and `git diff --check` all exited 0: 396 tests passed with one platform skip, plus Ruff, formatting, Bash, pairing, exact truncation, publication, and hosted checks. Calibration proved canonical push/support/visibility/contact/height/parked-arm bounds; all four reset images were inspected. Ignored manifest `outputs/scenarios_0827/calibration/20260828T235224.896890Z/manifest.json` SHA-256 `e3a207ba6cf1387c72d54908632900af72c6b161bcc6985dc99b332f05047fda`. | Rerun `make scenario-calibrate`, the stock/custom smokes in the suite overview, then `make ci && make secret-scan && make public-audit`; inspect the four calibration images and PR checks. |
| E-PC-S0827 | SP04, SP06–SP14 | Mac client→private loopback tunnel→WSL RTX inference; `2026-08-28T23:58Z`–`2026-08-29T00:08Z`; exact Mac/WSL implementation and upstream SHAs match E-MAC-S0827; PyTorch backend with automatic partial-BF16 loading. For each profile, `make server`, `make smoke-policy`, `make scenario-matrix`, `make scenario-metrics`, and `make stop` with the overview's pinned WSL/profile/backend variables all exited 0. The π₀ and experimental π₀.₅ batches each passed four scenarios × seeds 0–2: 24/24 infrastructure-valid episodes, 7,200/7,200 applied steps and trajectory rows, exact pairing/GPU coverage, 24/24 videos, and 24/24 plots. Every plot was inspected and contained all 14 actual-joint series plus commanded overlays; all video midpoints showed the correct scene. Telemetry-write p95 maxima were `0.1801643 ms` and `0.1897332 ms`, below 1 ms. All episodes ended at the 300-step limit with zero contact, lift, fall, off-table, or Push-PI success; task success is 0/24 while infrastructure is 24/24. Final stop and doctor found no owned listener, tunnel, server, or sampler. Ignored matrix summaries: `outputs/scenarios_0827/20260828T235859.466010Z/pi0_aloha_sim/matrix-summary.json` SHA-256 `0780026fdfbab2c48a5a9b756c1d48561583deb79ac319e918e7c18bb074dbf7`; `outputs/scenarios_0827/20260829T000331.820451Z/pi05_aloha_base/matrix-summary.json` SHA-256 `547e69b05de8b7d4ff17cc3579c80952238d92a16edad70608b5a86a2c2bdf50`. | With the exact implementation checked out, run one server/smoke/matrix/metrics/stop sequence for each profile as specified in the suite overview; inspect all summaries, videos, and plots. Do not run PC CI or display smokes. |
