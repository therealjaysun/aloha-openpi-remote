...
- **Implementation tasks:** Validate mapping keys; preserve and record stock Gym numeric finite `state (14,) float64`; require `cam_high` and allowed image names only, uint8 CHW `(3,224,224)`; validate optional prompt; validate exact wire response `actions (50,14)` as floating and finite; do not clip absolute policy joint commands to Gym's nominal box; verify profile/config/checkpoint/SHA metadata; preserve compliant arrays without redundant copies; produce actionable errors.
