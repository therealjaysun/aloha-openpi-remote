...
- **Acceptance:** Stock remains default. Every active episode MP4 is an ordered overhead/left-wrist/right-wrist strip with one frame per applied step. The opt-in 6,000-step π₀ Scenario 1 run uses exact orient/approach/push boundaries, flushes old buffered actions, records no prompt text, and remains separate from 300-step acceptance evidence.
