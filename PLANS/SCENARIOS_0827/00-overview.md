...
- **Acceptance:** Stock remains the default task and π₀.₅ is the default profile. Every active episode MP4 is an ordered overhead/left-wrist/right-wrist strip with one frame per applied step. Opt-in Scenario 1 runs may compare π₀.₅ base with the π₀ block-transfer checkpoint using the same 6,000-step ceiling and exact orient/approach/push schedule; each flushes old buffered actions, records no prompt text, preserves any earlier natural terminal, and remains separate from 300-step acceptance evidence.
- **Test commands:** Run focused three-view/staged tests, local lint/secret/public gates, and hosted checks. Then use `OPENPI_WSL_DISTRO=Ubuntu-24.04 OPENPI_POLICY_PROFILE=pi0_aloha_sim OPENPI_POLICY_BACKEND=pytorch` for doctor/setup/server/smoke; run one seed-0 headless Scenario 1 episode with `ALOHA_EPISODE_STEPS=6000 ALOHA_PROMPT_SCHEDULE=push_pi_single_left_staged_v1`; inspect metrics/artifacts, stop twice, and run final doctor. The two profile matrices remain historical.
...
- **Current status:** In progress on PR 8. Exact SHA `b1a1727` passed the π₀.₅ staged run, evidence, and cleanup; the requested same-seed/schedule π₀ block-transfer checkpoint comparison is the only active hardware item.
...
9. **Composite/staged extension:** Keep the matrix fixed. Validate triptych video and exact prompt boundaries locally/hosted, then run one Scenario 1 seed-0 diagnostic per selected profile under the 6,000-step ceiling. Inspect all three panels across stages, plot, telemetry linkage, coverage/timing, GPU metrics, and cleanup before marking SP19–SP20 complete.
