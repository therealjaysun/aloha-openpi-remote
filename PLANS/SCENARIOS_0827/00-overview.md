...
- **Acceptance:** Stock Transfer Cube remains the default; every normal policy request for stock and custom scenarios contains exactly real `cam_high`, `cam_left_wrist`, and `cam_right_wrist` uint8 CHW `(3,224,224)` views; existing state/action, pairing, projection, telemetry, artifact, and cleanup contracts remain unchanged. Focused local tests cover the global camera path, and one exact-candidate π₀ Scenario 1 episode measures three-view behavior without reclassifying the historical top-only matrices.
- **Test commands:** Current amendment: focused three-view tests, `make ci`, `make secret-scan public-audit`, hosted checks, then doctor/setup/server/smoke, one headless π₀ `push_pi_single` seed-0 `make run`, metrics, stop, idempotent stop, and final doctor. The two profile matrices remain historical baseline commands.
...
- **Current status:** Complete; open for review. The three-view implementation, local/hosted gates, exact-SHA π₀ Scenario 1 acceptance run, user-requested 120-second diagnostic, artifact inspection, and cleanup passed. Prior matrices, coverage, and long-run evidence remain historical top-only results.
- **Actual results:** All four Gym IDs compile/reset on the pinned Mac stack. The frozen descriptor is `7d41cc0ddfb88f46eb0c33139f8e82c92ccc823853e07a80a5413faf135ad2d9`. Four canonical 300-step pushes achieved `+0.117..+0.215 m` Y motion, exact named contact, at most `0.00638 m` height deviation, and at most `0.00882 rad` parked-arm error. Every randomized/canonical body and target exceeded 20 policy pixels; four reset images and four display-on smokes passed. The two historical hardware matrices completed 24/24 infrastructure-valid episodes, 7,200/7,200 steps and trajectory samples, and 24/24 videos and 14-joint plots, with zero task success. The exact π₀ coverage run completed 300/300 samples with `0.0%` coverage and no contact. The separate π₀.₅ diagnostic completed 6,000/6,000 samples over 120 simulated seconds with `0.0%` coverage; around step 960 the arm swept the π about 4.2 cm farther from its target, then remained near joint limits. The exact three-view π₀ run completed 300/300 steps, coverage rows, joint rows, and frames in `9.998114 s`, with no contact or coverage; all three named views, all 14 actual/commanded trajectory series, artifacts, GPU sampling, and cleanup passed. Compared descriptively with the historical top-only π₀ run, wall time rose from `6.968412 s` to `9.998114 s` and warmed-request p95 from `326.094 ms` to `586.980 ms`, without task improvement. The same three-view π₀ candidate then completed a separate 6,000-step/120-simulated-second diagnostic in `184.399483 s` wall time; coverage and named contact stayed zero while the left arm accumulated `16.830378 rad` joint travel and the π remained visually stationary.
...
- **Validated implementation SHA:** Three-view `6662a235278f01c6ee08dec976f58359c47181ec`; historical standalone diagnostic `6e6180c85b5ae279966f790ee7e96720b3285a6e`; coverage evaluator `4516422a95e3d3572997cace51b6a9b718cb8794`; matrix `7c2ec5927ad200e5aaf30bed0db4ef61cb9e2ba4`.
...
6. Keep `cam_high` as the sole post-step video/display frame; wrist views are policy inputs only. Complete or partial encoding is atomic and remains one frame per applied step.
