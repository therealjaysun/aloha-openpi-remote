...
- **Acceptance:** Stock remains the default task and π₀.₅ is the default profile. Every active episode MP4 is an ordered overhead/left-wrist/right-wrist strip with one frame per applied step. The opt-in 6,000-step π₀.₅ Scenario 1 run uses exact orient/approach/push boundaries, flushes old buffered actions, records no prompt text, and remains separate from 300-step acceptance evidence.
...
- **Current status:** Extension in progress on PR 8. Implementation tests pass; full gates, exact-SHA staged hardware run, artifact inspection, evidence, and cleanup remain.
...
9. **Composite/staged extension:** Keep the matrix fixed. Validate triptych video and exact prompt boundaries locally/hosted, then run one π₀.₅ Scenario 1 seed-0 6,000-step diagnostic. Inspect all three panels across stages, plot, telemetry linkage, coverage/timing, GPU metrics, and cleanup before marking SP19–SP20 complete.
