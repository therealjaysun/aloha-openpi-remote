...
- **Scope:** Stock Transfer Cube plus four fixed Push-PI scenario IDs; mandatory overhead and both wrists; one shared environment; fixed-gripper projection; fixed scenario prompts plus one opt-in Scenario 1 staged diagnostic; existing buffered control, JSONL, trajectories, composite videos, GPU sampling, tunnel, and optional Mac viewer.
...
- **Acceptance:** Stock remains the default task and π₀.₅ is the default profile. Every active episode MP4 is an ordered overhead/left-wrist/right-wrist strip with one frame per applied step. The π₀ block-transfer comparison uses the same seed, cameras, and 6,000-step ceiling as the π₀.₅ run but receives the complete look/lower/push instruction once through the normal fixed prompt; it preserves any earlier natural terminal and remains separate from 300-step acceptance evidence.
- **Test commands:** Run focused prompt/descriptor/three-view tests plus local lint/secret/public gates; GitHub Actions is disabled. Then use `OPENPI_WSL_DISTRO=Ubuntu-24.04 OPENPI_POLICY_PROFILE=pi0_aloha_sim OPENPI_POLICY_BACKEND=pytorch` for doctor/setup/server/smoke; run one seed-0 headless Scenario 1 episode with `ALOHA_EPISODE_STEPS=6000 ALOHA_PROMPT_SCHEDULE=fixed`; inspect metrics/artifacts, stop twice, and run final doctor. The two profile matrices remain historical.
...
- **Current status:** In progress on PR 8. Exact SHA `b1a1727` passed the historical π₀.₅ staged run, evidence, and cleanup; the requested same-seed/one-go-prompt π₀ block-transfer comparison is the only active hardware item.
- **Actual results:** Historical calibration/matrices/coverage and three-view π₀ evidence remain unchanged. On exact SHA `b1a1727`, π₀.₅ setup/smoke and the staged Scenario 1 run used all three cameras and prompt boundaries `0/500/1500`. The episode preserved 3,646 exact telemetry/trajectory/video samples before its natural off-table terminal at 72.92 simulated/115.65 wall seconds. Coverage stayed `0.0%`; the left arm spent most of the run near joint limits, then visually made non-finger hand contact at steps 3,640–3,646, lifting and moving the π away from its target. The inspected `224×672` MP4 contained all three ordered views and the plot contained all 14 actual/commanded series. Infrastructure, GPU coverage, safe summaries, live status, and cleanup passed; task success failed.
...
- **Validated implementation SHA:** Live-status/staged three-view `b1a1727e8aad06fb5592ecba94c2eb57fa406306`; historical three-view `6662a235278f01c6ee08dec976f58359c47181ec`; matrix `7c2ec5927ad200e5aaf30bed0db4ef61cb9e2ba4`.
...
| `push_pi_single` | `pi_robotics/PushPiSingleArm-v0` | One rigid π | Left six | `Using only the left arm, first tilt the wrist down to see the pi-shaped block and its matching outline, then lower the gripper close to the table beside the block and make short incremental pushes that move the block into the outline; recheck alignment after each push and do not lift the block.` |
...
9. **Prompt experiment:** Keep the matrix fixed. Validate the one-go fixed prompt and triptych locally, then run one π₀ block-transfer Scenario 1 seed-0 diagnostic under the 6,000-step ceiling. Inspect all three panels, plot, telemetry linkage, coverage/timing, GPU metrics, and cleanup before marking SP22 complete.
