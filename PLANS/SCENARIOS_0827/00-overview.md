...
- **Dependencies:** Completed phases 00–06; pinned OpenPI `215abfb217dbac7d5f1273282331b9b1866c0479`; installed `gym-aloha==0.1.1`; converted PyTorch π₀.₅ base and experimental Trossen block-transfer artifacts. Earlier π₀ evidence remains historical only.
...
- **Acceptance:** Stock remains the default task and π₀.₅ is the default profile. Every finite 14-D model action reaches MuJoCo unchanged in stock and all custom scenarios; “single arm” is prompted and measured, not software-enforced. Current checkpoint comparisons keep seed 0, three cameras, one fixed prompt, and exactly 6,000 steps for all four scenarios; they remain separate from 300-step acceptance evidence.
- **Test commands:** Run focused prompt/descriptor/three-view tests plus local lint/secret/public gates; GitHub Actions is disabled. Sync the exact candidate once, then run doctor/setup/server/smoke and one seed-0 headless 6,000-step fixed-prompt episode for each current π₀.₅ checkpoint/scenario. Inspect metrics/artifacts, stop at the end, and run final doctor. Historical matrices remain separate.
...
- **Current status:** Fixed-duration amendment complete on candidate `196f8e5`. Local gates, exact PC sync, checkpoint smoke, a fall-producing 10-step qualification, all eight π₀.₅ 6,000-step runs, artifact decoding, and cleanup passed. Earlier matrices remain historical evidence.
...
### Current 120-second checkpoint comparison

Every row is seed 0 with three cameras, 6,000 steps, 6,000 trajectory samples, and a decoded 6,000-frame/120.000-second MP4. All plots contain 14 actual and 14 commanded series. Best coverage was 0% in every run.

| π₀.₅ checkpoint | Scenario | Wall time | Server p95 | Recorded outcome evidence |
| --- | --- | ---: | ---: | --- |
| base | `push_pi_single` | 219.299 s | 215.361 ms | none |
| base | `push_pi_dual` | 198.642 s | 212.153 ms | none |
| base | `push_letters_single` | 232.880 s | 212.157 ms | right contact, lift, fall, off-table |
| base | `push_letters_dual` | 234.133 s | 215.485 ms | none |
| Trossen block-transfer | `push_pi_single` | 196.181 s | 224.534 ms | right contact, lift, fall, off-table |
| Trossen block-transfer | `push_pi_dual` | 198.637 s | 226.692 ms | right contact, lift, fall |
| Trossen block-transfer | `push_letters_single` | 230.460 s | 228.038 ms | left contact, lift, fall, off-table |
| Trossen block-transfer | `push_letters_dual` | 230.989 s | 224.621 ms | left contact, lift, fall, off-table |

The Trossen checkpoint is a third-party real-robot checkpoint, not a simulator or Push-PI fine-tune. Its immediate violent failures are evidence of an action/robot-domain mismatch, not useful task behavior.
...
| `push_pi_single` | `pi_robotics/PushPiSingleArm-v0` | One rigid π | Left arm requested; all 14 commands pass | `A PI-shaped block and a dotted target outline are on the table. Using only the left arm, push the block until it covers and aligns with the target outline.` |
| `push_pi_dual` | `pi_robotics/PushPiBimanual-v0` | One rigid π | Both arms requested; all 14 commands pass | `A PI-shaped block and a dotted target outline are on the table. Using both arms, push the block until it covers and aligns with the target outline.` |
| `push_letters_single` | `pi_robotics/PushLettersSingleArm-v0` | Separate rigid uppercase `P` and `I` | Left arm requested; all 14 commands pass | `P and I blocks and their dotted target outlines are on the table. Using only the left arm, push each block until it covers and aligns with its matching target outline.` |
| `push_letters_dual` | `pi_robotics/PushLettersBimanual-v0` | Separate rigid uppercase `P` and `I` | Both arms requested; all 14 commands pass | `P and I blocks and their dotted target outlines are on the table. Using both arms, push each block until it covers and aligns with its matching target outline.` |
...
- Keep 300 steps as the fixed acceptance-test limit. Every current checkpoint-comparison experiment uses a standalone 6,000-step run (120 simulated seconds) for each of the four scenarios; the matrix rejects that override and extended evidence is never pooled with acceptance results.
...
- `lifted_ever` becomes permanently true if COM rises more than `0.01 m`; it prevents success. Success, off-table, and fall evidence remain sticky, but none ends a custom episode: every valid run continues to its configured 300- or 6,000-step time limit so checkpoint comparisons have equal duration. Reward is `1` after held success, else `0`.
...
- Current work validates only π₀.₅: `pi05_aloha_base` plus the pinned experimental `pi05_trossen_block_transfer` real-robot checkpoint. The latter is not simulator- or Push-PI-fine-tuned, so this is a descriptive checkpoint comparison and zero task successes are allowed. Earlier π₀ evidence remains historical and is not rerun.
...
7. **Completion:** Inspect each selected video, trajectory plot, telemetry row linkage, coverage summary, and cleanup. Preserve partial artifacts, but complete the amendment only after every selected run reaches its exact configured time limit on the same candidate.
