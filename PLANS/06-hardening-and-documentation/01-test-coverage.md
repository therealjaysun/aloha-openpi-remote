...
- **Actual findings:** `make test` discovers 293 project tests: 292 passed and the Linux-only PIDfd test skipped on macOS. The suite covers T01–T12, including repository doctor/PR stack/public audit behavior and final cleanup/output-boundary regressions. Twenty-one feasible upstream `openpi-client` image/msgpack tests also passed. Ruff, formatting, and Bash syntax passed locally; both hosted pure-check jobs passed.
