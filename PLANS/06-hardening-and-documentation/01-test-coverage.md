...
- **Actual findings:** `make test` discovers 287 project tests: 286 passed and the Linux-only PIDfd test skipped on macOS. The suite covers T01–T12, including repository doctor/PR stack/public audit behavior added here. Twenty-one feasible upstream `openpi-client` image/msgpack tests also passed. Ruff, formatting, and Bash syntax passed locally; both hosted pure-check jobs passed.
