...
- **Actual findings:** The ControlMaster lifecycle and fail-closed state validation pass local focused tests. The first real tunnel bound only Mac `127.0.0.1` and returned four finite π₀ chunks, but the server later exited when Windows automatically stopped idle WSL. A separately created synchronous WSL client conclusively preserved the server and allowed the same smoke to pass. The integrated fix reuses the owned ControlMaster for that lifetime and adds no detached process, service, task, WSL configuration, firewall, or portproxy state.
