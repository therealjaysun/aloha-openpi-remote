...
- **Scope:** Remote discovery, effective WSL RAM/CUDA checks, pinned OpenPI install, profile selection, checkpoint cache, safe live Mac-terminal setup/server status, loopback server lifecycle, GPU evidence.
- **Non-goals:** Automatic Windows `.wslconfig`, driver/firewall/WSL network/SSH-server changes; training/fine-tuning; public model port; claiming π₀.₅ is sim-fine-tuned.
...
- **Acceptance criteria:** Detected distro (never guessed); effective WSL total/available RAM is valid and reported; Mac/WSL source SHAs match and contain the audited upstream base; only exact setup/server status is visible on the Mac while complete mode-600 evidence and marker validation remain intact; RTX 3090 visible; a WSL-local request proves both profiles start on `127.0.0.1:8000`, return finite `(50,14)` GPU actions, survive setup SSH exit, and stop safely; π₀.₅ limitation is explicit.
...
- **Actual results:** On the original 12 GiB WSL limit, the pinned JAX π₀ path OOMed on first inference and the stock converter exceeded RAM; the partial-BF16 path then passed both profiles. On 2026-08-29 the PC was upgraded to 48 GB and its user-owned WSL ceiling changed from 12 to 32 GB with 8 GB swap preserved; Ubuntu reported 32,866,932 KiB total and 31,981,904 KiB available. Exact-candidate setup/runtime revalidation is pending. Automatic conversion still selects partial BF16 below 16 GiB and full FP32 otherwise.
