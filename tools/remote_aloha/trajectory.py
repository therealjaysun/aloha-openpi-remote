...
def validate_joint_vector(value: object, label: str) -> list[float]:
...
def _trajectory_rows(events: Iterable[Mapping[str, object]], steps_applied: int) -> list[dict[str, object]]:
    ...
    for event in events:
        ...
        row: dict[str, object] = {
            ...
            "actual": validate_joint_vector(event.get("actual_joint_positions"), "actual_joint_positions"),
        }
        if has_command:
            row["commanded"] = validate_joint_vector(
                event.get("commanded_joint_positions"), "commanded_joint_positions"
            )
        rows.append(row)
...
def _summary(rows: list[dict[str, object]], steps_applied: int) -> dict[str, object]:
    return {
...
def summarize_trajectory(events: Iterable[Mapping[str, object]], steps_applied: int) -> dict[str, object]:
    return _summary(_trajectory_rows(events, steps_applied), steps_applied)
...
def write_trajectory_plot(
    events: Iterable[Mapping[str, object]], steps_applied: int, path: str | Path, plot_id: str
) -> dict[str, object]:
    rows = _trajectory_rows(events, steps_applied)
    summary = _summary(rows, steps_applied)
    if not rows:
        return summary
    ...
    actual = [_normalize(row["actual"]) for row in rows]  # type: ignore[arg-type]
    commanded = [_normalize(row["commanded"]) for row in rows] if "commanded" in rows[0] else []  # type: ignore[arg-type]
    colors = [
    ...
    axes.grid(alpha=0.2)
    axes.set_title("ALOHA joint trajectory")
    ...
    figure.text(
        0.08,
        0.025,
        "Arms use fixed gym-aloha 0.1.1 XML radian limits; grippers use its documented 0-1 close/open coordinate. "
        "50% is zero only for symmetric waist/roll joints.",
        fontsize=8,
        color="0.35",
    )
